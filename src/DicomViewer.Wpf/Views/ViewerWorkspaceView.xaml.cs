...
namespace DicomViewer.Wpf.Views;
...
public partial class ViewerWorkspaceView : UserControl
{
    ...
    {
        if (DataContext is not ViewerWorkspaceViewModel viewModel || !viewModel.HasSeriesItems)
        {
            return;
        }
    ...
    {
        if (DataContext is not ViewerWorkspaceViewModel viewModel || viewModel.ViewportImageSource is null || viewModel.ImagePixelWidth <= 0 || viewModel.ImagePixelHeight <= 0)
        {
            return null;
        }

        var point = e.GetPosition(ViewportImageCanvas);
        var maxX = Math.Max(0, viewModel.ImagePixelWidth - 1);
        var maxY = Math.Max(0, viewModel.ImagePixelHeight - 1);
        return new Point(
